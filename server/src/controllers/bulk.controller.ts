 ...
 */
export const importProductsFromExcel = asyncHandler(async (req: AuthRequest, res: Response) => {
    if (!req.file) {
    ...
    for (const row of data) {
        try {
            ...
            if (row.images) {
                productData.images = row.images.toString().split(',').map((img: string) => img.trim());
            }
 ...
 */
export const bulkUploadImages = asyncHandler(async (req: AuthRequest, res: Response) => {
    if (!req.files || !Array.isArray(req.files) || req.files.length === 0) {
        throw new ApiError(400, 'No images provided');
    }

    const files = req.files as Express.Multer.File[];
    const uploadResults = files.map(f => ({
        originalName: f.originalname,
        url: `/uploads/${f.filename}`,
        sku: f.originalname.split('.')[0]?.split('_')[0] // Extract SKU from filename (e.g. SKU123_1.jpg -> SKU123)
    }));

    const autoMatchedCount = 0;
    // Optional: Actually find and update products if SKU matches
