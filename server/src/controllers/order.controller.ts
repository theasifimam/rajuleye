...
export const createOrder = asyncHandler(async (req: AuthRequest, res: Response) => {
  const { shippingAddress, paymentMethod = 'cod', notes } = req.body as {
    shippingAddress: object; paymentMethod?: 'cod' | 'online'; notes?: string;
  };
  ...
  for (const item of cart.items) {
    const product = item.product as unknown as {
      _id: object; name: string; images: string[]; price: number; discount: number; stock: number; isActive: boolean;
    };
...
export const updateOrderStatus = asyncHandler(async (req: AuthRequest, res: Response) => {
  const { orderStatus, trackingNumber, paymentStatus } = req.body as {
    orderStatus?: string; trackingNumber?: string; paymentStatus?: string;
  };
