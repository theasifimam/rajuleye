...
export const getCategories = asyncHandler(async (req: AuthRequest, res: Response) => {
  const { all, parent } = req.query as Record<string, string | undefined>;
...
export const createCategory = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const data: any = { name, slug: categorySlug, isActive };
...
export const updateCategory = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  if (slug && slug !== category.slug) {
    const existing = await Category.findOne({ slug, _id: { $ne: id as any } });
    if (existing) throw new ApiError(400, 'Slug already in use');
    data.slug = slug;
  } else if (name && name !== category.name && !slug) {
    // Optionally auto-update slug if name changes and no new slug provided
