...
export const addToCart = asyncHandler(async (req: AuthRequest, res: Response) => {
  const { productId, qty = 1, lensType, lensCoating, selectedPower } = req.body as {
    productId: string; qty?: number; lensType?: string; lensCoating?: string[]; selectedPower?: object;
  };
  ...
  if (existingItem) {
    ...
    if (selectedPower) existingItem.selectedPower = selectedPower as typeof existingItem.selectedPower;
  } else {
    cart.items.push({ product: product._id as any, qty, priceAtAdd: finalPrice, lensType, lensCoating, selectedPower } as any);
  }
