...
export const sendEmail = async ({ to, subject, html }: SendEmailOptions): Promise<void> => {
  await transporter.sendMail({
    from: process.env.EMAIL_FROM || `"Rajul Eye" <noreply@rajuleye.com>`,
    to,
    subject,
    html,
  });
...
export const sendOTPEmail = async (to: string, otp: string, purpose: 'verify' | 'reset' | 'delete'): Promise<void> => {
