    ...
    if (urlCategorySlug !== urlSyncedSlug && dynamicCategories.length > 0) {
      ...
      if (urlCategorySlug) {
        const matchedCat = dynamicCategories.find(
          (c) =>
            c.slug === urlCategorySlug ||
            c.name.toLowerCase() === urlCategorySlug.toLowerCase() ||
            c.id === urlCategorySlug,
        );
        if (matchedCat) {
      ...
      <div className="flex items-center gap-3 mb-8 overflow-x-auto pb-4 hide-scroll px-1">
        <Sheet>
          <SheetTrigger asChild>
            <Button
              variant="outline"
              className="h-10 rounded-2xl gap-2 border-2 hover:bg-primary hover:text-primary-foreground hover:border-primary transition-all px-5 shadow-sm"
            >
