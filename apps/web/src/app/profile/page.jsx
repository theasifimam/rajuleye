...
export default function ProfilePage() {
    ...
    const [deletionStep, setDeletionStep] = useState('warn');
    useEffect(() => {
        if (!isLoggedIn) {
            router.push('/');
        }
    ...
    return (<div className="container mx-auto px-4 md:py-16 max-w-5xl relative z-10">
            {/* Header Section */}
                ...
                <div className="flex flex-col md:flex-row items-center gap-6 md:gap-8 bg-card rounded-[2rem] md:rounded-[3rem] p-6 md:p-12 border border-border/50 shadow-md relative overflow-hidden backdrop-blur-sm">
                    ...
                    <div className="relative group">
                        ...
                        <div className="relative w-32 h-32 md:w-40 md:h-40 bg-gradient-to-br from-primary to-primary/80 rounded-full flex items-center justify-center border-8 border-background shadow-md transform transition-transform group-hover:scale-105 duration-500 overflow-hidden">
                            {user.avatar ? (<img src={user.avatar} alt={user.name} className="h-full w-full object-cover" onError={(e) => {
                e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(user.name)}&background=bg-primary`;
                    ...
                    <div className="text-center md:text-left flex-1 space-y-3">
                        ...
                        <div className="flex items-center justify-center md:justify-start gap-3 md:gap-8 pt-2 overflow-x-auto">
                            ...
                            <div className="w-px h-6 md:h-8 bg-border/60 shrink-0"/>
                            <div onClick={() => setIsEyePowerOpen(true)} className="flex flex-col items-center md:items-start shrink-0 cursor-pointer hover:opacity-80 transition-opacity">
                    ...
                    <Button variant="outline" onClick={handleLogout} className="rounded-3xl px-8 h-12 md:self-start border-2 hover:bg-destructive hover:text-white hover:border-destructive transition-all duration-300 font-bold shadow relative z-[30] cursor-pointer pointer-events-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-16">
                {menuItems.map((item, index) => (<div key={index} onClick={item.action} className="group flex flex-col p-8 rounded-[2.5rem] border bg-card hover:bg-muted/50 hover:shadow-lg hover:-translate-y-1 transition-all duration-500 cursor-pointer relative overflow-hidden">
            ...
            <UpdateProfileDialog isOpen={isUpdateOpen} onOpenChange={setIsUpdateOpen} onOpenDeletion={(step) => {
            ...
            <LogoutConfirmDialog isOpen={isLogoutConfirmOpen} onOpenChange={setIsLogoutConfirmOpen} onConfirm={confirmLogout}/>
            <DeleteAccountDialog isOpen={isDeleteAccountOpen} onOpenChange={setIsDeleteAccountOpen} initialStep={deletionStep}/>
        </div>);
}
