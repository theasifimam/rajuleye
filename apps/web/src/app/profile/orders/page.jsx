                ...
                {orders.length > 0 ? (orders.map((order) => (<div key={order._id} className="bg-card rounded-[2.5rem] border border-border/50 shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-500 group">
                            ...
                            <div className="p-6 md:p-8 bg-muted/30 border-b flex flex-wrap items-center justify-between gap-4">
                                ...
                                <div className="flex items-center gap-6">
                                    ...
                                    <div className={cn("px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-[0.1em] flex items-center gap-2", order.orderStatus === 'delivered' ? "bg-emerald-500/10 text-emerald-600" : "bg-orange-500/10 text-orange-600")}>
                            ...
                            <div className="p-6 md:p-8 space-y-6">
                                {order.items.map((item, idx) => (<Link key={idx} href={`/product/${item.product}`} className="flex gap-4 group/item hover:bg-muted/40 p-3 -m-3 rounded-3xl transition-all duration-300">
