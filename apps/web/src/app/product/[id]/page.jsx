...
export default async function ProductPage({ params }) {
  ...
  try {
    ...
    const relRes = await fetch(`${apiBase}/api/v1/products`, {
      next: { revalidate: 0 },
    });
    const relJson = await relRes.json();
    const relatedProducts = relJson.data.products
      .map((rp) => ({
        ...rp,
        id: rp._id,
        ...
        discountPrice: rp.discount
          ? rp.price - (rp.price * rp.discount) / 100
          : undefined,
        category:
          typeof rp.category === "object" && rp.category
            ? rp.category.name
            : rp.category,
      }))
      .filter((rp) => rp.id !== product.id && rp.category === product.category)
      .slice(0, 4);
    return (
      <ProductDetailClient
        product={product}
