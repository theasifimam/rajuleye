...
export function ProductTabs({ product, reviews, isLoggedIn, hasBoughtProduct, orderId, user, }) {
    return (<div className="pt-6">
            <Tabs defaultValue="details" className="w-full">
                <TabsList className="bg-transparent border-b border-muted-foreground/10 w-full justify-start rounded-none h-12 p-0 gap-4 xs:gap-8 overflow-x-auto scrollbar-hide">
                    {["details", "shipping", "reviews"].map((tab) => (<TabsTrigger key={tab} value={tab} className="relative h-full rounded-none bg-transparent px-4 text-[10px] font-black uppercase tracking-[0.2em] data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:after:absolute data-[state=active]:after:bottom-0 data-[state=active]:after:left-0 data-[state=active]:after:h-[2px] data-[state=active]:after:w-full data-[state=active]:after:bg-primary transition-all capitalize">
            ...
            "AR Nano-Coating"
        ].map((text, i) => (<li key={i} className="flex items-center gap-3 text-[10px] font-black uppercase tracking-widest text-primary list-none">
            ...
            { label: "Support", val: "24/7" }
        ].map((stat, i) => (<div key={i} className="space-y-1 p-4 rounded-2xl bg-muted/30 border border-border/50">
                ...
                <TabsContent value="reviews" className="pt-6 px-4">
                    <ReviewSection product={product} reviews={reviews} isLoggedIn={isLoggedIn} hasBoughtProduct={hasBoughtProduct} orderId={orderId} user={user}/>
                </TabsContent>
            </Tabs>
        </div>);
}
