...
export function ReviewForm({ user, productId, orderId }: ReviewFormProps) {
    ...
    if (hasSubmittedRate) {
        return (
            <div className="py-12 px-6 rounded-[2.5rem] bg-emerald-500/5 border border-emerald-500/20 text-center animate-in fade-in zoom-in-95 duration-500">
                ...
                <Button
                    ...
                    onClick={() => setHasSubmittedRate(false)}
                >
                    Post another review
                </Button>
            </div>
        ...
        <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="flex gap-2">
                    {[1, 2, 3, 4, 5].map((star) => (
                        ...
                        >
                            <Star
                                className={cn(
                                    "w-8 h-8 xs:w-10 xs:h-10 transition-all duration-300",
            ...
            <div className="space-y-4">
                ...
                <div className="relative">
                    <Textarea
                        placeholder="Describe the fit, lens clarity, and overall feel..."
                        className="min-h-[160px] rounded-[2.5rem] bg-muted/20 border-2 border-transparent focus:border-primary/20 focus:bg-background transition-all font-medium text-sm p-6 pr-12 resize-none shadow-inner"
                        value={reviewText}
            ...
            <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-4">
                    {reviewImages.map((img, i) => (
                        <div key={i} className="relative h-20 w-20 xs:h-24 xs:w-24 rounded-2xl overflow-hidden border border-border group/img shadow-sm hover:shadow-md transition-shadow">
                            <Image src={img.preview} alt="Preview" fill className="object-cover transition-transform group-hover/img:scale-110" />
                            <button
                                type="button"
                                onClick={() => removeImage(i)}
                                className="absolute top-2 right-2 h-6 w-6 bg-black/60 backdrop-blur-md rounded-full flex items-center justify-center text-white opacity-0 group-hover/img:opacity-100 transition-all"
                            >
                                <X className="h-3 w-3" />
                            </button>
                        </div>
                    ...
                    {reviewImages.length < 4 && (
                        <label className="h-20 w-20 xs:h-24 xs:w-24 rounded-2xl border-2 border-dashed border-muted-foreground/10 hover:border-primary/30 hover:bg-primary/5 transition-all cursor-pointer flex flex-col items-center justify-center gap-2 group">
            ...
            <Button
                ...
                disabled={isLoading}
                className="w-full h-16 rounded-2xl font-black text-[10px] uppercase tracking-[0.3em] shadow-2xl shadow-primary/20 group relative overflow-hidden bg-primary hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
