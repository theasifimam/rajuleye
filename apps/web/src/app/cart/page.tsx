            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 md:gap-12">
                <div className="lg:col-span-2 space-y-6">
                    <ul className="space-y-6">
                        {items.map((item) => (
                            <li key={item.product?.id || (item.product as any)?._id} className="p-4 md:p-6 bg-card rounded-[2rem] border-none shadow-md hover:shadow-lg transition-shadow flex flex-col sm:flex-row gap-4 md:gap-8 items-start sm:items-center">
                                ...
                                <div className="flex flex-col flex-1 min-w-0 w-full h-full justify-between self-stretch py-2">
                                    <div className="flex justify-between items-start gap-4 mb-4">
                                        <div className="space-y-1">
                                            <Link href={`/product/${item.product?.id || (item.product as any)?._id}`} className="font-extrabold text-lg md:text-xl line-clamp-2 hover:text-primary transition-colors tracking-tight">
                                                {item.product?.name}
                                            </Link>
                                    ...
                                    <div className="flex items-center justify-between mt-auto">
                                        <div className="flex items-center bg-muted/50 rounded-full border border-border/50 shadow-sm px-1 py-1 h-12">
                                            <Button
                                                ...
                                                onClick={() => handleUpdateQuantity(item.product?.id || (item.product as any)?._id, item.qty - 1)}
                                                disabled={item.qty <= 1}
                                            >
                                        ...
                                        <Button
                                            ...
                                            onClick={() => setIsRemoving(item.product?.id || (item.product as any)?._id)}
                                        >
