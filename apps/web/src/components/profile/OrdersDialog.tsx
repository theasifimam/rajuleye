...
export function OrdersDialog({ isOpen, onOpenChange }: OrdersDialogProps) {
    ...
    return (
        <ResponsiveModal
            ...
            title={selectedOrder ? 'Tracking Detail' : 'Order History'}
            className="sm:max-w-[700px] border border-border/50"
            headerClassName="h-auto bg-transparent border-b border-border/20 text-foreground items-start py-6 px-8 [&>div.absolute]:hidden"
            contentClassName="w-full items-stretch p-6 sm:p-8 custom-scrollbar"
        >
            <div className="space-y-4">
                {selectedOrder && (
                    <button
                        onClick={() => setSelectedOrder(null)}
                        className="flex items-center gap-2 mb-4 text-[10px] font-black uppercase tracking-widest text-primary hover:opacity-70 transition-opacity"
                    >
                ...
                {!selectedOrder ? (
                        ...
                        {isLoading ? (
                            ...
                            orders.map((order) => (
                                <div
                                    ...
                                    className="group bg-muted/20 rounded-[2rem] border border-border/50 p-6 shadow-sm hover:shadow-xl hover:scale-[1.01] transition-all cursor-pointer relative overflow-hidden"
                                >
                                    ...
                                    <div className="flex items-center gap-3 mb-4">
                                        <div className="flex -space-x-4 overflow-hidden">
                                            {order.items.slice(0, 3).map((item, idx) => (
                                                <div key={idx} className="h-12 w-12 rounded-2xl bg-muted border-4 border-background flex items-center justify-center overflow-hidden shadow-inner">
                                                    <img src={item.image} alt={item.name} className="h-full w-full object-cover" />
                                                </div>
                                            ))}
                                            {order.items.length > 3 && (
                                                <div className="h-12 w-12 rounded-2xl bg-primary text-white border-4 border-background flex items-center justify-center text-xs font-bold leading-none">
                                                    +{order.items.length - 3}
                                                </div>
                                            )}
                                        </div>
                    ...
                    <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
                        ...
                        <div className="space-y-3">
                            ...
                            {selectedOrder.items.map((item, idx) => (
                                <Link
                                    ...
                                    className="flex items-center gap-4 p-4 rounded-2xl bg-muted/20 border border-border/50 hover:bg-muted/40 transition-all group/item"
                                >
                                ...
                                return (
                                    <div key={idx} className="relative flex gap-6 pb-8 last:pb-0">
                                        <div className={cn(
                                            ...
                                            isCompleted ? "bg-primary text-white" : "bg-muted text-muted-foreground border-2 border-border"
                                        )}>
                                            {isCompleted ? (
                        ...
                        <Button
                            onClick={() => setSelectedOrder(null)}
                            className="w-full h-16 rounded-full font-black text-sm uppercase tracking-widest shadow-xl"
                        >
                            Back to History
                        </Button>
                    </div>
