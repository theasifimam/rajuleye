...
interface DeleteAccountDialogProps {
    ...
    initialStep?: 'warn' | 'otp';
}

export function DeleteAccountDialog({ isOpen, onOpenChange, initialStep = 'warn' }: DeleteAccountDialogProps) {
    const [step, setStep] = useState<'warn' | 'otp'>(initialStep);
            ...
            <div className="w-full flex flex-col items-center gap-6 py-2">
                    ...
                    <div className="w-full space-y-4 px-2">
                        <div className="relative">
                            <Input
                                ...
                                className="h-16 text-center text-3xl font-black tracking-[0.5em] rounded-2xl border-2 border-muted bg-muted/30 focus-visible:ring-destructive focus-visible:border-destructive transition-all placeholder:tracking-normal placeholder:text-muted-foreground/30"
                                autoFocus
                            />
                        </div>
                ...
                <div className="w-full flex flex-col sm:flex-row gap-3 mt-4">
                    <Button
                        ...
                        className="rounded-2xl flex-1 font-bold h-14 order-2 sm:order-1 hover:bg-muted"
                    >
                        Cancel
                    </Button>
                    {step === 'warn' ? (
                        <Button
                            ...
                            onClick={handleRequestOTP}
                            disabled={isRequesting}
                            className="rounded-2xl flex-1 font-black uppercase tracking-widest h-14 shadow-xl shadow-destructive/20 order-1 sm:order-2 active:scale-[0.98] transition-transform"
                        >
