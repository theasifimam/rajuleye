...
import { Home, Search, ShoppingBag, User, LayoutGrid, Heart } from 'lucide-react';
...
export function BottomNav() {
    ...
    return (
        <div className="fixed bottom-6 left-0 right-0 z-[100] flex justify-center px-6 lg:hidden pointer-events-none">
                ...
                {navItems.map((item) => {
                    ...
                    return (
                        <Link
                            ...
                            className={cn(
                                "relative flex flex-col items-center justify-center h-12 flex-1 rounded-full transition-all duration-500",
                                isActive
                                    ? "text-primary"
                        ...
                        >
                            ...
                            {isActive && (
                                <div className="absolute inset-x-1 inset-y-1 bg-primary/10 rounded-full -z-10 animate-in fade-in zoom-in duration-500" />
                            )}

                            <div className="relative">
                                <div className="h-5 w-5 rounded-full overflow-hidden transition-all duration-500">
                                    {item.name === 'Profile' && isLoggedIn && user?.avatar ? (
                                        <img
                                            src={user.avatar}
                                            alt={user.name}
                                ...
                                {item.badge ? (
                                    <span className="absolute -top-2 -right-2 bg-primary text-primary-foreground text-[7px] font-black min-w-[14px] h-3.5 px-1 rounded-full flex items-center justify-center ring-2 ring-black dark:ring-white shadow-sm z-10 transition-all duration-300">
                                        {item.badge}
                                    </span>
                                ) : null}
                            </div>
