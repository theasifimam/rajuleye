...
export function Navbar({ settings }) {
  ...
  const wishlistItemsCount = isLoggedIn
    ? wishlistData?.data?.products?.length || 0
    : 0;
  // Get database cart count
  const { data: cartData } = useGetCartQuery(undefined, { skip: !isLoggedIn });
  const cartItemsCount = isLoggedIn
    ? cartData?.data?.items?.reduce((sum, item) => sum + item.qty, 0) || 0
    : 0;
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-100 pointer-events-none lg:px-6">
      ...
      >
            ...
            <div className="flex items-baseline gap-1 px-4 py-1">
              {settings?.logo ? (
                ...
                <>
                  ...
                  <span className="text-sm font-light uppercase tracking-tighter opacity-40">
                    Eye
                  </span>
                </>
              )}
        ...
        <Link
          href="/"
          className="absolute left-1/2 -translate-x-1/2 hidden lg:flex items-center gap-3 group focus:outline-none pointer-events-auto"
        >
          {settings?.logo ? (
            <img
              src={getLogoUrl(settings.logo)}
        ...
        <div className="flex items-center gap-4 pointer-events-auto">
          ...
          <div className="lg:hidden">
            <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
              ...
              <SheetContent
                side="right"
                className="w-full sm:w-[400px] p-0 border-l-none"
              ...
              >
                <div className="flex flex-col h-full bg-background relative overflow-hidden z-50">
                  ...
                  <div className="p-8 flex items-center justify-between relative z-10">
                    {settings?.logo ? (
                      ...
                      <div className="flex items-baseline gap-1 select-none">
                        ...
                        <span className="text-2xl font-light uppercase tracking-tighter opacity-40">
                          Eye
                        </span>
                      </div>
                    )}
                    <div className="flex items-center gap-2">
                      ...
                      <SheetClose asChild>
                        <Button
                          variant="ghost"
                          size="icon"
                          className="h-12 w-12 rounded-full bg-muted/50 hover:bg-primary hover:text-primary-foreground backdrop-blur-md transition-all shadow-sm"
                        >
                          <X className="h-5 w-5" />
                        </Button>
                      </SheetClose>
                  ...
                  <nav className="flex-1 px-8 py-8 space-y-2 overflow-y-auto">
                    ...
                    <div className="space-y-2 pt-4">
                      <button
                        ...
                        className="w-full flex items-center justify-between p-4 rounded-2xl bg-muted/30 hover:bg-muted/50 transition-all group"
                      >
                        <div className="flex items-center gap-4">
                          <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center text-primary">
                        ...
                        <ChevronRight
                          className={cn(
                            "w-5 h-5 opacity-40 transition-transform duration-300",
                            isCategoriesOpen && "rotate-90 opacity-100",
                          )}
                        />
                      </button>

                      {isCategoriesOpen && (
                        <div className="grid grid-cols-1 gap-2 pl-4 animate-in slide-in-from-top-4 duration-300">
                          {[
                            {
                              name: "Optical Frames",
                              href: "/search?category=optical",
                            },
                            {
                              name: "Sunglasses",
                              href: "/search?category=sunglasses",
                            },
                            {
                              name: "Blue Light",
                              href: "/search?category=blue-light",
                            },
                            {
                            ...
                            >
                              <span className="text-sm font-bold uppercase tracking-wider opacity-60 group-hover:opacity-100">
                                {item.name}
                              </span>
                    ...
                    <div className="pt-8 space-y-2">
                      <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 ml-4 mb-2">
                        Your Space
                      </p>
                      <Link
                        href="/profile"
                  ...
                  <div className="p-8 pb-12 mt-auto">
                    {isLoggedIn ? (
                      <div className="flex items-center gap-4 p-4 rounded-3xl bg-primary text-primary-foreground shadow-xl">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-black truncate uppercase tracking-tight">
                            {user?.name}
                          </p>
                      ...
                      <Button
                        ...
                        className="w-full h-16 rounded-[2rem] bg-primary font-black text-sm uppercase tracking-widest shadow-2xl flex items-center justify-center gap-3"
                      >
