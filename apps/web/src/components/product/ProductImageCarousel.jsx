...
export function ProductImageCarousel({ images, name }) {
    ...
    const displayImages = validImages.length > 0
        ? validImages.map(img => img.startsWith('http') || img.startsWith('/') || img.startsWith('data:') ? img : `/${img}`)
        : [
    ...
    return (<div className="relative w-full group">
                ...
                <CarouselContent>
                    {displayImages.map((image, index) => (<CarouselItem key={index}>
                            <div className="relative aspect-square w-full md:rounded-[2.5rem] rounded-none overflow-hidden bg-muted transition-all duration-300">
                                <Image src={image} alt={`${name} - Image ${index + 1}`} fill priority={index === 0} className="object-cover transition-transform duration-500 group-hover:scale-110" sizes="(max-width: 768px) 100vw, 50vw"/>
                            </div>
                        </CarouselItem>))}
                </CarouselContent>
            ...
            {displayImages.length > 1 && (<div className="flex gap-2 mt-4 justify-center overflow-x-auto pb-2">
                    {displayImages.map((image, index) => (<button key={index} onClick={() => scrollTo(index)} className={cn("relative w-16 h-16 rounded-lg overflow-hidden border-2 transition-all flex-shrink-0 active:scale-95", current === index ? "border-primary shadow-md" : "border-transparent grayscale opacity-60 hover:opacity-100 hover:grayscale-0")}>
                            <Image src={image} alt={`${name} thumb ${index + 1}`} fill className="object-cover" sizes="64px"/>
                        </button>))}
                </div>)}

            {/* Dots for mobile quick view */}
            {displayImages.length > 1 && (<div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5 px-3 py-1.5 rounded-full bg-black/20 backdrop-blur-sm pointer-events-none sm:hidden">
                    {displayImages.map((_, index) => (<div key={index} className={cn("h-1.5 rounded-full transition-all duration-300", current === index ? "w-6 bg-white" : "w-1.5 bg-white/50")}/>))}
                </div>)}
        </div>);
}
