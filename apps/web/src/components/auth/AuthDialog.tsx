...
type AuthStep = 'auth' | 'verify' | 'forgot' | 'reset';
...
function saveSessionState(state: PersistedState) {
    try {
...
export function AuthDialog() {
    ...
    const handleVerifyEmail = async () => {
        if (formData.otp.length < 4) { toast.error('Please enter the verification code'); return; }
        try {
    ...
    const handleForgotPassword = async () => {
        if (!formData.email || !/^\S+@\S+\.\S+$/.test(formData.email)) { toast.error('Please enter your email'); return; }
        try {
    ...
    const getDescription = () => {
        if (step === 'verify') return 'Enter the code sent to your email.';
        if (step === 'forgot') return "We'll send a reset code to your email.";
