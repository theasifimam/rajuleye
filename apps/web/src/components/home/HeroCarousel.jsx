...
export function HeroCarousel() {
    ...
    return (<div className="relative w-full mt-[-112px] lg:mt-[-128px]">
                                    ...
                                    <div className={cn("absolute inset-0 bg-gradient-to-r flex items-center pt-20 md:pt-32 from-black/80 via-black/40 to-transparent")}>
                                        <div className="container px-6 md:px-12 mx-auto max-w-[1600px]">
                                            <div className="max-w-2xl space-y-6 text-white animate-in fade-in slide-in-from-left-8 duration-1000">
                                                {slide.label && (<div className="inline-block py-1.5 px-4 rounded-full bg-white/10 backdrop-blur-xl border border-white/10 text-[10px] font-black uppercase tracking-[0.3em] mb-4">
                                                        {slide.label}
                                                    </div>)}
                                                ...
                                                {slide.buttonLink && (<div className="pt-6 flex gap-4">
            ...
            <div className="absolute bottom-10 left-12 flex gap-4 z-20">
                {(banners.length > 0 ? banners : [1]).map((_, index) => (<button key={index} onClick={() => api?.scrollTo(index)} className={cn("h-1 transition-all duration-700 rounded-full", current === index ? "w-16 bg-primary" : "w-4 bg-white/20 hover:bg-white/40")} aria-label={`Go to slide ${index + 1}`}/>))}
            </div>
        </div>);
}
