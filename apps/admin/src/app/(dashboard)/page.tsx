...
export default function DashboardPage() {
  ...
  if (error || !dashboardData) {
    return (
      <div className="flex flex-col h-full w-full items-center justify-center min-h-[500px] text-destructive gap-4">
  ...
  return (
    <div className="space-y-8 md:space-y-12 pb-10">
      <PageHeader
        ...
        titleMain="The Collection"
        titleAccent="Perspective"
        description="Curating high-precision optical heritage. Your administrative command center is synchronized and ready."
        showAction={false}
      >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 px-4 md:px-0">
        {stats.map((stat, i) => (
          <div key={i} className="group p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] bg-card border hover:border-primary/50 transition-all duration-500 shadow-sm hover:shadow-md relative overflow-hidden">
            <div className="flex items-start justify-between mb-6 md:mb-8">
              ...
              <div className={cn(
                ...
                stat.isPositive ? "bg-primary/10 text-primary" : "bg-destructive/10 text-destructive"
              )}>
        ...
        <div className="lg:col-span-8 p-6 md:pt-10 md:px-10 md:pb-8 rounded-[2.5rem] md:rounded-[3.5rem] bg-card border shadow-sm relative overflow-hidden group flex flex-col">
          ...
          <div className="flex-1 min-h-[200px] md:min-h-[260px] flex items-end gap-1.5 md:gap-3 justify-between pt-2 overflow-x-auto scrollbar-none">
            {monthlyRevenue.map((h, i) => (
              <div key={i} className="min-w-[12px] flex-1 flex flex-col items-center gap-3 md:gap-4 group/bar relative h-full">
                ...
                <div className="w-full flex-1 flex items-end">
                  <div className="w-full bg-muted/20 rounded-t-lg md:rounded-t-xl relative overflow-hidden h-full group-hover/bar:bg-muted/30 transition-colors duration-500">
                    <div
                      className="absolute inset-x-0 bottom-0 bg-primary/40 group-hover/bar:bg-primary transition-all duration-700 rounded-t-lg md:rounded-t-xl shadow-[0_0_15px_rgba(34,197,94,0.2)]"
                      style={{ height: `${h}%` }}
                    />
            ...
            {recentOrders && recentOrders.length > 0 ? recentOrders.map((order, i) => (
              <div key={i} className="flex items-center gap-4 md:gap-5 group cursor-pointer">
                <div className={cn(
                  ...
                  order.status === 'delivered' ? "bg-primary/5 text-primary border-primary/10" : "bg-muted/50 text-muted-foreground border-transparent"
                )}>
                ...
                <div className="flex-1 min-w-0 text-sm">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className={cn(
                      ...
                      order.status === 'delivered' ? "bg-primary/10 text-primary border-primary/20" : "bg-muted text-muted-foreground border-transparent"
                    )}>
                      {order.status}
                    </span>
                  </div>
              ...
              <p className="text-xs text-muted-foreground text-center pt-10 font-bold">No recent orders found.</p>
            )}
          </div>
