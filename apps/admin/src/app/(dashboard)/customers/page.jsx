...
import { Search, Edit2, Trash2, CheckCircle2, XCircle, TrendingUp, Shield, ShieldAlert, ShieldCheck, User as UserIcon, Send, } from 'lucide-react';
...
export default function UsersPage() {
    ...
    const filteredUsers = useMemo(() => {
        ...
        return users.filter(user => {
            const matchesSearch = user.name?.toLowerCase().includes(lowSearch) ||
    ...
    if (isError) {
        return (<div className="flex h-[400px] flex-col items-center justify-center gap-4 text-destructive text-center">
    ...
    return (<div className="space-y-8 md:space-y-12 pb-10">
            <PageHeader badgeIcon={Shield} badgeText="Identity Command" titleMain="The Citizen" titleAccent="Registry" description="Curating the global community of Rajul Eye ambassadors. Managing privileges and profiles with architectural precision.">
                ...
                <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                    <Button onClick={() => setIsBroadcastDialogOpen(true)} variant="signature" size="xl" className="h-16 md:h-20 w-full sm:w-auto">
                    ...
                    <Button onClick={openAddDialog} variant="signature" size="xl" className="h-16 md:h-20 w-full sm:w-auto">
                        <div className="flex flex-col items-center gap-0.5 md:gap-1">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6 px-4 md:px-0">
                {pulseStats.map((stat, i) => (<div key={i} className="p-5 md:p-6 rounded-[1.5rem] md:rounded-[2rem] bg-card border shadow-sm flex items-center gap-4 md:gap-5 group hover:border-primary/20 transition-all">
                        <div className={cn("h-10 w-10 md:h-12 md:w-12 rounded-2xl md:rounded-2xl flex items-center justify-center shrink-0 transition-transform group-hover:scale-110", stat.color === 'primary' ? "bg-primary text-primary-foreground" :
            ...
            <div className="flex flex-col md:flex-row items-center gap-4 p-4 rounded-[2rem] bg-card/50 border border-primary/5 backdrop-blur-md mx-4 md:mx-0">
                <div className="relative flex-1 w-full group">
                    ...
                    <Input placeholder="Search by name, email or registry ID..." className="h-11 md:h-12 w-full pl-12 pr-4 bg-muted/20 border-none rounded-2xl font-bold text-sm focus-visible:ring-2 focus-visible:ring-primary/20" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}/>
                </div>
                <div className="flex gap-2 w-full md:w-auto">
                    <div className="flex bg-muted/20 p-1 rounded-2xl border border-primary/5 items-center">
                        {['all', 'user', 'moderator', 'admin', 'subscribers'].map((role) => (<button key={role} onClick={() => setRoleFilter(role)} className={cn("px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all", roleFilter === role ? "bg-primary text-primary-foreground shadow-lg" : "text-muted-foreground hover:text-primary")}>
                                {role}
                            </button>))}
                    </div>
                </div>
            ...
            <div className="rounded-[3rem] bg-card border shadow-md border-primary/5 overflow-hidden relative mx-4 md:mx-0">
                <div className="overflow-x-auto">
                    {roleFilter === 'subscribers' ? (<table className="w-full text-left">
                            <thead>
                            ...
                            <tbody className="divide-y divide-primary/5">
                                {filteredSubscribers.map((sub, idx) => (<tr key={sub._id} className="group hover:bg-primary/[0.03] transition-all duration-500">
                                ...
                                {filteredSubscribers.length === 0 && (<tr>
                            ...
                            <tbody className="divide-y divide-primary/5">
                                {filteredUsers.map((user) => (<tr key={user._id} className="group hover:bg-primary/[0.03] transition-all duration-500">
                                        <td className="p-8">
                                            <div className="relative h-14 w-14 mx-auto">
                                                ...
                                                <div className="relative h-full w-full rounded-2xl overflow-hidden border border-white/10 shadow-sm bg-muted flex items-center justify-center z-10">
                                                    {user.avatar ? (<img src={user.avatar} alt={user.name} className="h-full w-full object-cover"/>) : (<span className="text-xs font-black text-primary uppercase">{user.name?.split(' ').map(n => n[0]).join('')}</span>)}
                                                </div>
                                            </div>
                                        </td>
                                        <td className="p-8">
                                        ...
                                        <td className="p-8 text-center">
                                            <div className={cn("inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-[9px] font-black uppercase tracking-[0.2em] border shadow-sm", user.role === 'admin' ? "bg-primary text-primary-foreground border-primary shadow-primary/20" :
                        ...
                        "bg-muted/50 text-muted-foreground border-primary/5")}>
                                        ...
                                        <td className="p-8">
                                            <div className="flex items-center gap-2">
                                                {user.isEmailVerified ? (<div className="flex items-center gap-1.5 text-primary">
                                                        <CheckCircle2 className="h-3.5 w-3.5"/>
                                                        <span className="text-[9px] font-black uppercase tracking-widest">Verified</span>
                                                    </div>) : (<div className="flex items-center gap-1.5 text-muted-foreground/40">
                                        ...
                                        <td className="p-8">
                                            <div className="flex flex-col">
                                                ...
                                                <div className="flex items-center gap-1.5">
                                                    <div className={cn("h-1.5 w-1.5 rounded-full", user.isActive ? "bg-primary shadow-[0_0_8px_rgba(34,197,94,1)]" : "bg-muted-foreground/30")}/>
                                                    <span className={cn("text-[9px] font-black uppercase tracking-widest", user.isActive ? "text-primary" : "text-muted-foreground/50")}>{user.isActive ? 'Active' : 'Standby'}</span>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="p-8 text-right">
                                            <div className="flex items-center justify-end gap-2 translate-x-4 opacity-0 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-500">
                                                <Button variant="ghost" size="icon" onClick={() => openEditDialog(user)} className="h-12 w-12 rounded-2xl hover:bg-primary/10 hover:text-primary border border-transparent hover:border-primary/20 transition-all">
                                                    <Edit2 className="h-4 w-4"/>
                                                </Button>
                                                <Button variant="ghost" size="icon" disabled={isDeleting} onClick={() => handleDelete(user._id)} className="h-12 w-12 rounded-2xl hover:bg-destructive/10 hover:text-destructive border border-transparent hover:border-destructive/20 transition-all">
                ...
                <UserDialog isOpen={isDialogOpen} onOpenChange={setIsDialogOpen} user={editingUser}/>
