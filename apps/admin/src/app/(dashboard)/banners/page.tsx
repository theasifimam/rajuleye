...
import { useGetBannersQuery, useDeleteBannerMutation, useUpdateBannerMutation, type Banner } from '@/store/bannerApi';
    ...
    return (
        <div className="space-y-8 md:space-y-12 pb-10">
            ...
            <PageHeader
                ...
                titleMain="Marketing"
                titleAccent="Visuals"
                description="Orchestrating the brand narrative. Curating the visual stories that define the Rajul Eye aesthetic across the digital horizon."
            >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6 px-4 md:px-0">
                {campaignStats.map((stat, i) => (
                    <div key={i} className="p-6 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] bg-card border shadow-sm border-primary/5 hover:border-primary/20 transition-all flex items-center justify-between group">
                        <div className="flex items-center gap-4 md:gap-6">
                            <div className={cn(
                                ...
                                stat.color === 'primary' ? "bg-primary text-primary-foreground shadow-primary/20" : "bg-muted text-muted-foreground shadow-sm"
                            )}>
            ...
            {isLoading ? (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 px-4 md:px-0">
                    {banners.map((banner) => (
                        <div key={banner.id} className="group rounded-[3rem] bg-card border shadow-sm border-primary/5 overflow-hidden flex flex-col hover:shadow-xl transition-all duration-700">
                            {/* Banner Preview Image */}
                            <div className="relative h-72 overflow-hidden bg-muted">
                                <img
                                    src={banner.image.startsWith('http') ? banner.image : banner.image.startsWith('/') ? banner.image : `/${banner.image}`}
                                    alt={banner.title}
                                ...
                                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex flex-col justify-end p-8">
                                    <div className="flex items-center gap-2 mb-2">
                                        <span className={cn(
                                            ...
                                            banner.status === 'Active' ? "bg-primary/20 text-primary border-primary/30" :
                                ...
                                <div className="flex items-center gap-3 w-full sm:w-auto pt-4 sm:pt-0 border-t sm:border-t-0 border-primary/5 sm:justify-end">
                                    ...
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            banner.status === 'Active' ? "bg-primary shadow-[0_0_10px_rgba(34,197,94,0.3)]" : "bg-muted"
                                        )}>
                                        <div className={cn(
