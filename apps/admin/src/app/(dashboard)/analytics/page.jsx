  ...
  return (
    <div className="space-y-8 md:space-y-12 pb-10">
      <PageHeader
        ...
        description="Track your store's performance, revenue, and customer behavior."
      >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 px-4 md:px-0">
        {kpiStats.map((stat, i) => (
          <div
            key={i}
            className="p-6 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] bg-card border shadow-sm border-primary/5 hover:border-primary/10 transition-all relative overflow-hidden group"
          >
            {/* Background Accent */}
            <div
              className={cn(
                "absolute -right-4 -top-4 w-16 h-16 md:w-24 md:h-24 blur-2xl md:blur-[60px] opacity-20 group-hover:opacity-40 transition-opacity",
                stat.color === "primary"
                  ? "bg-primary"
                  : stat.color === "blue"
                    ? "bg-blue-500"
            ...
            <div className="flex items-start justify-between mb-6 md:mb-8 relative z-10">
              <div
                className={cn(
                  "h-10 w-10 md:h-12 md:w-12 rounded-xl md:rounded-2xl flex items-center justify-center shadow-inner transition-transform group-hover:scale-110",
                  stat.color === "primary"
              ...
              <div
                className={cn(
                  "px-1 rounded-full text-[8px] md:text-[9px] font-black tracking-widest flex items-center gap-1 uppercase",
                  stat.isPositive
            ...
            <div className="relative z-10">
              ...
              <h3 className="text-xl md:text-3xl font-black tracking-tighter italic leading-none">
                {stat.value}
              </h3>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between mb-8 md:mb-12">
            ...
            <div className="flex items-center gap-3 md:gap-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[9px] md:text-[10px] font-black uppercase tracking-widest opacity-60 truncate max-w-[60px] md:max-w-none">
                  Revenue
                </span>
              </div>
              <Button
                variant="ghost"
                size="icon"
                className="h-9 w-9 md:h-10 md:w-10 rounded-xl bg-muted/20"
              >
            ...
            <div className="flex items-end justify-between gap-2 md:gap-4 pt-4 min-w-[600px] md:min-w-0 h-full">
              {[45, 60, 40, 85, 55, 75, 50, 90, 65, 95, 80, 88].map(
                (val, i) => (
                  <div
                    key={i}
                    className="flex-1 flex flex-col items-center gap-3 md:gap-4 group/bar relative"
                  >
                    ...
                    <div className="w-full bg-muted/20 rounded-t-xl md:rounded-t-2xl relative overflow-hidden transition-all duration-700 h-[240px] md:h-[300px] flex items-end">
                      <div
                        className="w-full bg-primary/30 group-hover/bar:bg-primary transition-all duration-700 ease-out rounded-t-xl md:rounded-t-2xl relative shadow-[0_0_20px_rgba(34,197,94,0.2)]"
                        style={{ height: `${val}%` }}
                      >
                        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-px md:w-[2px] h-full bg-white/20" />
                      </div>
                    </div>
      ...
      <div className="rounded-[3rem] bg-card border shadow-sm border-primary/5 overflow-hidden mx-4 md:mx-0">
        <div className="p-10 border-b border-primary/5 bg-primary/1 flex items-center justify-between">
          <div>
            <h3 className="text-xl font-black tracking-tight uppercase italic leading-none mb-1">
              Top Products
            </h3>
          ...
          <Button
            variant="outline"
            className="h-12 px-8 rounded-2xl font-black uppercase tracking-widest text-[10px] border-2"
          >
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b bg-muted/5">
                ...
                <th className="p-8 text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground/60 w-48">
                  Sales Performance
                </th>
            ...
            <tbody className="divide-y divide-primary/5">
              {topProducts.map((p, i) => (
                ...
                >
                  ...
                  <td className="p-8 text-[10px] font-black uppercase tracking-widest text-muted-foreground opacity-60 italic">
                    {p.category}
                  </td>
                  <td className="p-8">
