    ...
    return (
        <div className="space-y-8 md:space-y-12 pb-10">
            <PageHeader
                ...
                description="Decoding the metrics of excellence. Transforming complex brand signals into clear strategic foresight for the Rajul Eye dynasty."
            >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 px-4 md:px-0">
                {kpiStats.map((stat, i) => (
                    <div key={i} className="p-6 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] bg-card border shadow-sm border-primary/5 hover:border-primary/10 transition-all relative overflow-hidden group">
                        {/* Background Accent */}
                        <div className={cn(
                            "absolute -right-4 -top-4 w-16 h-16 md:w-24 md:h-24 blur-[40px] md:blur-[60px] opacity-20 group-hover:opacity-40 transition-opacity",
                            stat.color === 'primary' ? "bg-primary" : stat.color === 'blue' ? "bg-blue-500" : stat.color === 'purple' ? "bg-purple-500" : "bg-orange-500"
                        )} />
                        ...
                        <div className="flex items-start justify-between mb-6 md:mb-8 relative z-10">
                            <div className={cn(
                                ...
                                stat.color === 'primary' ? "bg-primary text-primary-foreground shadow-primary/20" : "bg-muted text-foreground shadow-sm"
                            )}>
                                <stat.icon className="h-4 w-4 md:h-5 md:w-5" />
                            </div>
                            <div className={cn(
                                "px-2 px-1 rounded-full text-[8px] md:text-[9px] font-black tracking-widest flex items-center gap-1 uppercase",
                                stat.isPositive ? "bg-primary/10 text-primary border border-primary/20" : "bg-red-500/10 text-red-500 border border-red-500/20"
                            )}>
                    ...
                    <div className="flex-1 overflow-x-auto scrollbar-hide -mx-2 px-2">
                        <div className="flex items-end justify-between gap-2 md:gap-4 pt-4 min-w-[600px] md:min-w-0 h-full">
                            {[45, 60, 40, 85, 55, 75, 50, 90, 65, 95, 80, 88].map((val, i) => (
                                <div key={i} className="flex-1 flex flex-col items-center gap-3 md:gap-4 group/bar relative">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-primary/5">
                            {topProducts.map((p, i) => (
                                <tr key={i} className="group hover:bg-primary/[0.03] transition-all duration-500">
                                    <td className="p-8">
