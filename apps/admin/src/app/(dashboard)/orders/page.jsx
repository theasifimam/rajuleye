...
import { Search, Filter, Eye, Download, Truck, CheckCircle2, Clock, XCircle, CreditCard, User, ShoppingBag, TrendingUp, Activity, Loader2 } from 'lucide-react';
...
export default function OrdersPage() {
    ...
    const orders = React.useMemo(() => {
        ...
        return rawOrders.filter(o => o._id.toLowerCase().includes(lowSearch) ||
    ...
    return (<div className="space-y-8 md:space-y-12 pb-10 animate-in fade-in duration-700">
            <PageHeader badgeIcon={ShoppingBag} badgeText="Fulfillment Console" titleMain="Order" titleAccent="Logistics" description="Monitoring the lifecycle of luxury. From order inception to signature delivery, every shipment is a promise kept." showAction={false}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 px-4 md:px-0">
                {pulseStats.map((stat, i) => (<div key={i} className="p-5 md:p-6 rounded-[1.5rem] md:rounded-[2rem] bg-card border shadow-sm border-primary/5 flex items-center gap-4 group hover:border-primary/20 transition-all">
                        <div className={cn("h-10 w-10 md:h-12 md:w-12 rounded-xl md:rounded-2xl flex items-center justify-center shrink-0 transition-transform group-hover:scale-110", stat.color === 'primary' ? "bg-primary text-primary-foreground" : "bg-muted text-foreground")}>
            ...
            <div className="flex flex-col lg:flex-row items-center gap-4 p-4 rounded-[2rem] bg-card/50 border border-primary/5 backdrop-blur-md mx-4 md:mx-0">
                <div className="relative flex-1 w-full group">
                    ...
                    <Input placeholder="Search by order ID, customer name, or email..." className="h-12 w-full pl-12 pr-4 bg-muted/20 border-none rounded-xl font-bold text-sm focus-visible:ring-2 focus-visible:ring-primary/20" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}/>
                </div>
                <div className="flex gap-2 w-full lg:w-auto overflow-x-auto pb-2 lg:pb-0 scrollbar-none">
                    {['All Status', 'placed', 'confirmed', 'shipped', 'delivered', 'cancelled'].map((filter) => (<button key={filter} onClick={() => setFilterStatus(filter)} className={cn("px-5 h-12 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap border-2", filterStatus === filter
            ...
            return (<tr key={order._id} className="group hover:bg-primary/[0.03] transition-all duration-500">
                                        <td className="p-8">
                                        ...
                                        <td className="p-8">
                                            <div className="flex items-center gap-4">
                                                <div className="h-10 w-10 rounded-2xl bg-gradient-to-br from-muted to-muted/50 flex items-center justify-center shrink-0 border border-white/10 shadow-sm relative overflow-hidden group-hover:scale-110 transition-transform">
                                                    {order.user?.name ? (<span className="font-black text-primary capitalize">{order.user.name[0]}</span>) : (<User className="h-4 w-4 text-primary"/>)}
                                        ...
                                        <td className="p-8">
                                            <div className={cn("inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-[9px] font-black uppercase tracking-[0.2em] border", statusStyles[order.orderStatus])}>
                                        ...
                                        <td className="p-8">
                                            <div className="flex items-center gap-2">
                                                <div className={cn("h-2 w-2 rounded-full", order.paymentStatus === 'paid' ? "bg-primary shadow-[0_0_8px_rgba(34,197,94,1)]" : "bg-orange-500")}/>
                                        ...
                                        <td className="p-8 text-right">
                                            <Button variant="ghost" size="icon" className="h-12 w-12 rounded-xl hover:bg-primary/10 hover:text-primary border border-transparent hover:border-primary/20 transition-all group/eye" onClick={() => { setSelectedOrder(order); setIsDialogOpen(true); }}>
                ...
                <div className="p-6 md:p-8 border-t border-primary/5 bg-primary/[0.01] flex flex-col sm:flex-row items-center justify-between gap-6">
                    <div className="flex items-center gap-4 md:gap-5">
                        ...
                        <div>
                            ...
                            <h4 className="text-lg md:text-xl font-black italic">₹{orders.reduce((acc, order) => acc + order.finalAmount, 0) / orders.length}</h4>
                        </div>
                    </div>
                    <Pagination currentPage={page} totalPages={response?.data?.pagination?.pages || 1} onPageChange={setPage}/>
                </div>
            </div>


            <OrderDetailsDialog order={selectedOrder} open={isDialogOpen} onOpenChange={setIsDialogOpen}/>
        </div>);
}
