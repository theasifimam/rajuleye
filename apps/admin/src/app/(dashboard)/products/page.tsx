...
const ProductDialog = dynamic(() => import('@/components/admin/ProductDialog').then(mod => mod.ProductDialog), { ssr: false });
const BulkImportDialog = dynamic(() => import('@/components/admin/BulkImportDialog').then(mod => mod.BulkImportDialog), { ssr: false });
import { cn } from '@/lib/utils';
import { useGetProductsQuery, useDeleteProductMutation, type Product } from '@/store/productApi';
...
export default function ProductsPage() {
    ...
    const products = React.useMemo(() => productsData?.data?.products || [], [productsData]);
    const pagination = React.useMemo(() => productsData?.data?.pagination, [productsData]);

    const stats = React.useMemo(() => [
        { label: 'Stock Yield', value: 'Verified', icon: Boxes, color: 'primary' },
        { label: 'Out of Stock SKUs', value: 'Monitoring', icon: XCircle, color: 'destructive' },
        { label: 'Low Stock Alerts', value: 'Active', icon: AlertCircle, color: 'orange' },
    ], []);
    ...
    if (isError) {
        return (
            <div className="flex h-[400px] flex-col items-center justify-center gap-4 text-destructive">
                ...
                <p className="font-bold uppercase tracking-widest text-[10px]">Failed to load products</p>
                {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
        ...
        <div className="space-y-8 md:space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700 outline-none">
            ...
            >
                <div className="h-16 md:h-20 px-6 md:px-8 rounded-2xl md:rounded-[2rem] bg-card/80 backdrop-blur-md border-2 border-primary/10 shadow-sm flex flex-col justify-center gap-0.5 md:gap-1 min-w-[180px] md:min-w-[200px] hover:border-primary/30 transition-all duration-500">
                    <p className="text-[8px] md:text-[9px] font-black uppercase tracking-[0.2em] text-muted-foreground whitespace-nowrap">Active SKU Count</p>
                    <div className="flex items-end justify-between">
                        <h4 className="text-xl md:text-2xl font-black italic leading-none">{pagination?.total || 0}</h4>
                ...
                >
                    <div className="flex flex-col items-center gap-0.5 md:gap-1">
                        ...
                        <span className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] md:tracking-[0.25em]">Bulk Archive Import</span>
                    </div>
                </Button>
                <Button
                ...
                >
                    <div className="flex flex-col items-center gap-0.5 md:gap-1">
                        ...
                        <span className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] md:tracking-[0.25em]">Add Signature Piece</span>
                    </div>
                </Button>
            </PageHeader>
                    ...
                    <div key={i} className="p-5 md:p-6 rounded-[1.5rem] md:rounded-[2rem] bg-card border shadow-sm flex items-center gap-4 md:gap-5 group hover:border-primary/20 transition-all">
                        <div className={cn(
                            ...
                            stat.color === 'primary' ? "bg-primary text-primary-foreground" :
                                stat.color === 'destructive' ? "bg-destructive/10 text-destructive" : "bg-orange-500/10 text-orange-600"
                        )}>
            ...
            <div className="rounded-[3rem] bg-card border shadow-md border-primary/5 overflow-hidden relative mx-4 md:mx-0">
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="border-b bg-muted/10">
                                <th className="p-4 md:p-6 text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground/60 w-24 text-center">Preview</th>
                                <th className="p-4 md:p-6 text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground/60">Product DNA</th>
                                <th className="p-4 md:p-6 text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground/60">Category</th>
                                <th className="p-4 md:p-6 text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground/60">Stock Status</th>
                                <th className="p-4 md:p-6 text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground/60">MSRP</th>
                                <th className="p-4 md:p-6 text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground/60">Visibility</th>
                                <th className="p-4 md:p-6 text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground/60 text-right">Actions</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-primary/5">
                            {products.map((product) => (
                                <tr key={product._id} className="group hover:bg-primary/[0.03] transition-all duration-500">
                                    <td className="p-4 md:p-6">
                                        <div className="relative h-20 w-20 mx-auto">
                                            ...
                                            <div className="relative h-full w-full rounded-2xl overflow-hidden border border-white/10 shadow-sm bg-muted shrink-0 z-10">
                                                {product.images?.[0] ? (
                                                    <img src={product.images[0]} alt={product.name} className="h-full w-full object-cover group-hover:scale-125 transition-transform duration-700" />
                                                ) : (
                                    ...
                                    <td className="p-4 md:p-6">
                                        <div className="max-w-[200px]">
                                            <p className="font-black text-[13px] uppercase tracking-tight line-height-1 mb-1 leading-none">{product.name}</p>
                                    ...
                                    <td className="p-4 md:p-6">
                                        <div className="flex flex-col gap-2 min-w-[120px]">
                                            <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-widest">
                                                <span className={cn(
                                                    product.stock > 10 ? "text-primary" : product.stock > 0 ? "text-orange-500" : "text-destructive"
                                            ...
                                            <div className="h-1.5 w-full bg-muted rounded-full overflow-hidden">
                                                <div
                                                    className={cn(
                                                        ...
                                                        product.stock > 10 ? "bg-primary shadow-[0_0_8px_rgba(34,197,94,0.5)]" :
                                                            product.stock > 0 ? "bg-orange-500" : "bg-destructive shadow-[0_0_8px_rgba(239,68,68,0.5)]"
                                                    )}
                                    ...
                                    <td className="p-4 md:p-6">
                                        <span className="text-sm font-black italic tracking-tighter">${product.price.toFixed(2)}</span>
                                    </td>
                                    <td className="p-4 md:p-6">
                                        <div className={cn(
                                            "inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-[9px] font-black uppercase tracking-[0.2em] border",
                                            product.isActive ? "bg-primary/5 text-primary border-primary/20" : "bg-destructive/5 text-destructive border-destructive/20"
                                        )}>
                                            <div className={cn(
                                                "h-1.5 w-1.5 rounded-full animate-pulse",
                                                product.isActive ? "bg-primary shadow-[0_0_8px_rgba(34,197,94,1)]" : "bg-destructive shadow-[0_0_8px_rgba(239,68,68,1)]"
                                            )} />
                ...
                <div className="p-4 md:p-6 border-t border-primary/5 bg-primary/[0.01] flex flex-col sm:flex-row items-center justify-between gap-6">
                    <p className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground text-center sm:text-left">Catalog sync: {products.length} verified units from {pagination?.total || 0} total</p>
                    <Pagination
                        currentPage={page}
