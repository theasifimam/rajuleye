...
export function SpecsTab({ control }: SpecsTabProps) {
    return (
                    ...
                    render={({ field }) => (
                        <FormItem className="space-y-3">
                            ...
                            <Select onValueChange={field.onChange} value={field.value || ""}>
                                ...
                                <SelectContent className="rounded-2xl border-primary/10">
                                    {['round', 'square', 'rectangle', 'oval', 'cat-eye', 'wayfarer', 'aviator', 'clubmaster'].map(s => (
                    ...
                    render={({ field }) => (
                        <FormItem className="space-y-3">
                            ...
                            <Select onValueChange={field.onChange} value={field.value || ""}>
                                ...
                                <SelectContent className="rounded-2xl border-primary/10">
                                    {['metal', 'acetate', 'tr90', 'wood', 'titanium', 'mixed'].map(m => (
                    ...
                    render={({ field }) => (
                        <FormItem className="space-y-3">
                            ...
                            <Select onValueChange={field.onChange} value={field.value || ""}>
                                ...
                                <SelectContent className="rounded-2xl border-primary/10">
                                    {['single-vision', 'bifocal', 'progressive', 'non-prescription'].map(l => (
                                        <SelectItem key={l} value={l} className="capitalize">{l.replace('-', ' ')}</SelectItem>
                                    ))}
                                </SelectContent>
                            </Select>
                            <FormMessage />
