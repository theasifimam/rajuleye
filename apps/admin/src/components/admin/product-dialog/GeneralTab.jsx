...
export function GeneralTab({ control, categories }) {
    return (<div className="space-y-6 animate-in fade-in slide-in-from-left-4 duration-500">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <FormField control={control} name="category" render={({ field }) => (<FormItem className="space-y-3">
                            ...
                            <Select key={categories.length > 0 ? 'loaded' : 'loading'} onValueChange={field.onChange} value={field.value || ""}>
                                <FormControl>
                                ...
                                <SelectContent className="rounded-2xl border-primary/10">
                                    {categories.map((cat) => (<SelectItem key={cat._id} value={String(cat._id)}>
                                            {cat.name}
                                        </SelectItem>))}
                                </SelectContent>
                            </Select>
                ...
                <FormField control={control} name="slug" render={({ field }) => (<FormItem className="space-y-3">
                ...
                <FormField control={control} name="sku" render={({ field }) => (<FormItem className="space-y-3">
                ...
                <FormField control={control} name="brand" render={({ field }) => (<FormItem className="space-y-3">
            ...
            <FormField control={control} name="description" render={({ field }) => (<FormItem className="space-y-3">
                        ...
                        <FormControl>
                            <Textarea placeholder="Describe the artistic vision and technical prowess of this piece..." className="min-h-[100px] rounded-2xl bg-muted/30 border-none font-medium focus-visible:ring-2 focus-visible:ring-primary/20 p-5 resize-none transition-all" {...field}/>
                        </FormControl>
                        <FormMessage className="text-[10px] uppercase font-black tracking-widest"/>
                    </FormItem>)}/>
        </div>);
