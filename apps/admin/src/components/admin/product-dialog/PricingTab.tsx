...
export function PricingTab({ control }: PricingTabProps) {
    return (
            ...
            <FormField
                ...
                render={({ field }) => (
                    <FormItem className="space-y-3">
                        ...
                        <FormControl>
                            <Input
                                placeholder="e.g. vintage, collection, limited"
                                value={field.value?.join(', ') || ''}
                                onChange={(e) => field.onChange(e.target.value.split(',').map(t => t.trim()).filter(Boolean))}
                                className="h-11 rounded-xl bg-muted/30 border-none font-medium px-5 transition-all"
                            />
                        </FormControl>
                        <FormMessage />
                    </FormItem>
                        ...
                        <FormItem className="flex items-center space-x-3 space-y-0">
                            <FormControl>
                                <Checkbox
                                    ...
                                    className="h-6 w-6 rounded-lg border-primary data-[state=checked]:bg-primary data-[state=checked]:text-primary-foreground"
                                />
                            </FormControl>
