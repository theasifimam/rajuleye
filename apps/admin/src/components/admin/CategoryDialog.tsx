...
import { CategoryFormSchema, type CategoryFormValues } from '@/lib/zod-schemas';
import { type Resolver } from 'react-hook-form';
import { useCreateCategoryMutation, useUpdateCategoryMutation, type Category } from '@/store/categoryApi';
...
export function CategoryDialog({ isOpen, onOpenChange, category, categories }: CategoryDialogProps) {
    ...
    useEffect(() => {
        if (category && isOpen) {
            form.reset({
                ...
                parent: (category.parent as any)?._id || (typeof category.parent === 'string' ? category.parent : null),
                                ...
                                <div className="space-y-8">
                                    <div className="grid grid-cols-1 gap-6">
                                        ...
                                        <FormField
                                            ...
                                            render={({ field }) => (
                                                <FormItem className="space-y-3">
                                                    ...
                                                    <FormControl>
                                                        <div className="flex gap-2">
                                                            <div className="relative group/field flex-1">
                                                                ...
                                                                <Input
                                                                    placeholder="https://assets.rajuleye.com/..."
                                                                    {...field}
                                                            ...
                                                            <Button
                                                                ...
                                                                variant="outline"
                                                                className="shrink-0 h-11 px-4 rounded-xl border-primary/20 hover:bg-primary/5"
                                                            >
                                                    ...
                                                    {imagePreview && (
                                                        <div className="mt-3 relative h-32 w-32 rounded-xl overflow-hidden border border-primary/10">
                                                            <img
                                                                src={imagePreview.startsWith('http') || imagePreview.startsWith('/') || imagePreview.startsWith('data:') ? imagePreview : `/${imagePreview}`}
                                                                alt="Preview"
                                                    ...
                                                    <Select onValueChange={field.onChange} defaultValue={field.value || "none"}>
                                                        ...
                                                        <SelectContent className="rounded-xl border-primary/10 shadow-2xl">
                                                            ...
                                                            {categories.filter(c => c.id !== category?.id).map((c) => (
                                                                <SelectItem key={c.id} value={c.id} className="rounded-lg text-[10px] font-black uppercase tracking-widest">{c.name}</SelectItem>
                                                            ))}
                                                        </SelectContent>
                                                    </Select>
                                                    <FormMessage className="text-[10px] uppercase font-black tracking-widest" />
                                        ...
                                        <FormField
                                            ...
                                            name="isActive"
                                            render={({ field }) => (
                                                <FormItem className="flex flex-row items-center space-x-3 space-y-0 p-4 rounded-xl bg-muted/20 border border-primary/5">
                                                    <FormControl>
                                                        <Checkbox
                                                            checked={field.value}
