...
import { UserFormSchema, type UserFormValues } from '@/lib/zod-schemas';
import { GeneralTab } from './user-dialog/GeneralTab';
import { AddressTab } from './user-dialog/AddressTab';
import { EyePowerTab } from './user-dialog/EyePowerTab';
...
interface UserDialogProps {
    ...
    user?: (Partial<UserFormValues> & { _id?: string, id?: string }) | null;
}
...
export function UserDialog({ isOpen, onOpenChange, user }: UserDialogProps) {
    ...
    const onSubmit = async (values: UserFormValues) => {
        ...
        try {
            ...
            toast.error((err as { data?: { message?: string } })?.data?.message || 'Verification failed');
        }
    };
                            ...
                            <ScrollArea className="flex-1 h-full w-full overflow-y-auto px-8 md:px-12 py-6">
                                <Tabs defaultValue="general" className="w-full p-1">
                                    <TabsList className="bg-muted/30 p-1 rounded-2xl mb-8 w-fit border border-primary/5">
                                        ...
                                        <TabsTrigger value="vision" className="rounded-xl px-6 py-2 text-[10px] font-black uppercase tracking-widest data-[state=active]:bg-primary data-[state=active]:text-primary-foreground transition-all">
                                            Vision
                                        </TabsTrigger>
                                    </TabsList>
                                    ...
                                    <div className="mt-2">
                                        ...
                                        <TabsContent value="vision">
                                            <EyePowerTab control={form.control} />
                                        </TabsContent>
                                    </div>
                                </Tabs>
                            </ScrollArea>
