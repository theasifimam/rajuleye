      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="space-y-3">
              <FormLabel
                tooltip="The guest's full identity."
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="space-y-3">
              <FormLabel
                tooltip="Primary communication channel."
                className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground ml-1"
              >
                Email Address
              </FormLabel>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="space-y-3">
              <FormLabel
                tooltip="Secure access credential."
                className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground ml-1"
              >
                Password
              </FormLabel>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="space-y-3">
              <FormLabel
                tooltip="Administrative privilege level."
                className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground ml-1"
              >
                Registry Role
              </FormLabel>
              ...
              >
                ...
                <SelectContent className="rounded-xl border-primary/10 shadow-2xl">
                  ...
                  <SelectItem
                    value="admin"
                    className="rounded-lg text-[10px] font-black uppercase tracking-widest text-primary"
                  >
                    Admin
                  </SelectItem>
                </SelectContent>
          ...
          render={({ field }) => (
            <FormItem className="space-y-3 flex flex-col">
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <FormControl>
                    <Button
                      variant={"outline"}
                      className={cn(
                        "h-11 w-full pl-3 text-left font-normal rounded-xl bg-muted/30 border-none focus-visible:ring-2 focus-visible:ring-primary/20 transition-all",
                ...
                <PopoverContent
                  className="w-auto p-0 z-200 rounded-3xl border-primary/10 shadow-2xl"
                  align="start"
                >
                  <Calendar
                    mode="single"
          ...
          render={({ field }) => {
            ...
            return (
              <FormItem className="space-y-3 col-span-1 md:col-span-2">
                <FormLabel
                  tooltip="The guest's visual identity."
                  className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground ml-1"
                >
                  Profile Picture
                </FormLabel>
                ...
                {previewUrl && (
                  <div className="mt-3 relative h-24 w-24 rounded-2xl overflow-hidden border border-primary/10 bg-muted/20">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img
                      src={
                        previewUrl.startsWith('http') || previewUrl.startsWith('/') || previewUrl.startsWith('data:') || previewUrl.startsWith('blob:')
                          ? previewUrl
                          : `/${previewUrl}`
                      }
                      alt="Preview"
