...
export function EyePowerTab({ control }) {
    const eyeFields = [
    ...
    return (<div className="space-y-10 animate-in fade-in slide-in-from-right-4 duration-500">
            {['left', 'right'].map((side) => (<div key={side} className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
                        {eyeFields.map((field) => (<FormField key={field.name} control={control} name={`eyePower.${side}.${field.name}`} render={({ field: formField }) => (<FormItem className="space-y-2">
                                        <FormLabel className="text-[9px] font-black uppercase tracking-widest text-muted-foreground ml-1">{field.label}</FormLabel>
                                        <FormControl>
                                            <Input type="number" step="0.25" placeholder="0.00" {...formField} className="h-10 rounded-xl bg-muted/30 border-none font-black text-center focus-visible:ring-2 focus-visible:ring-primary/20"/>
                                        </FormControl>
