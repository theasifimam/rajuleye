...
export function Topbar() {
    ...
    return (<div className="h-20 bg-background/80 backdrop-blur-xl border-b sticky top-0 z-40 px-4 md:px-8 flex items-center justify-between transition-colors duration-500 gap-4">
            <div className="flex items-center gap-2 md:gap-4 flex-1 max-w-xl group relative">
                ...
                <Input type="text" placeholder="Search..." className="w-full h-11 md:h-12 bg-muted/50 border-none rounded-2xl pl-12 pr-4 text-[13px] md:text-sm font-bold focus-visible:ring-2 focus-visible:ring-primary/20 transition-all outline-none"/>
            </div>

            <div className="flex items-center gap-2 md:gap-4">
                {/* Theme Toggle */}
                <Button variant="ghost" size="icon" onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} className="h-11 w-11 md:h-12 md:w-12 rounded-2xl hover:bg-primary/5 transition-colors group relative shrink-0">
                ...
                <DropdownMenu>
                        ...
                        <div className="flex items-center gap-2 md:gap-3 pl-1 md:pl-2 group shrink-0 cursor-pointer">
                            ...
                            <div className="h-11 w-11 md:h-12 md:w-12 rounded-2xl bg-primary/20 p-[1.5px] md:p-[2px] flex items-center justify-center transition-all group-hover:bg-primary/30 overflow-hidden ring-2 ring-transparent group-hover:ring-primary/20">
                                <div className="h-full w-full rounded-[0.8rem] md:rounded-[0.9rem] bg-card border-2 border-primary/50 flex items-center justify-center overflow-hidden shadow-[inset_0_0_10px_rgba(34,197,94,0.1)]">
                                    {user?.avatar ? (<img src={user.avatar} alt={user.name} className="h-full w-full object-cover"/>) : (<User className="h-5 w-5 md:h-6 md:w-6 text-primary"/>)}
                                </div>
                            </div>
                        </div>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent className="w-56" align="end">
                        ...
                        <DropdownMenuSeparator />
                        <DropdownMenuItem onClick={() => setIsLogoutDialogOpen(true)} className="text-destructive focus:bg-destructive focus:text-white">
                ...
                <LogoutConfirmDialog isOpen={isLogoutDialogOpen} onOpenChange={setIsLogoutDialogOpen} onConfirm={handleLogout}/>
            </div>
        </div>);
}
